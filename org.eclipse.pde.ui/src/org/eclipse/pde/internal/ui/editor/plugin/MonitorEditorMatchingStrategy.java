...
public class MonitorEditorMatchingStrategy implements IEditorMatchingStrategy {
	...
	public boolean matches(IEditorReference editorRef, IEditorInput input) {
		...
		if (input instanceof IFileEditorInput && inputFile != null) {
			try {
				...
				if (inputFile.getName().endsWith(".mon") || inputFile.getName().endsWith(".MON")) {//$NON-NLS-1$ //$NON-NLS-2$
					return true;
				}
